  ...
  if (filled === "half") {
    return (
      ...
      >
        ...
        <path
          d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.618 4.992h5.253c.97 0 1.371 1.24.588 1.81l-4.243 3.127 1.618 4.992c.3.921-.755 1.688-1.538 1.116L10 15.347l-4.243 3.127c-.782.572-1.838-.195-1.538-1.116l1.618-4.992-4.243-3.127c-.783-.57-.382-1.81.588-1.81h5.253l1.618-4.992z"
          fill="url(#halfGrad)"
        />
      </svg>
    );
    ...
    <div className="flex items-center gap-1 relative">
      <div className="absolute flex items-center gap-1">
        {Array(5)
          .fill("")
          .map((_, i) => (
            <Star key={`empty-${i}`} filled={Filled.empty} />
          ))}
      </div>
      {Array(fullStars)
        .fill("")
        .map((_, i) => (
          <Star key={`full-${i}`} filled={Filled.full} />
        ))}
      {Array(halfStars)
        .fill("")
        .map((_, i) => (
          <Star key={`half-${i}`} filled={Filled.half} />
        ))}
      {Array(emptyStars)
        .fill("")
        .map((_, i) => (
      ...
      <span className="text-xs text-gray-400">{`${rating.toFixed(2)}`}</span>
      {reviews && (
